...
当前速度上下界设为无穷，因此 Gymnasium checker 会给出提醒。这不违反契约，但复杂训练中通常会根据物理极限设置有限范围，或对速度进行裁剪、归一化。
...
\[
a_t=\operatorname{clip}(K_p e_t,-1,1),\qquad K_p=0.5
\]
...
- 当前任务是一维、无噪声、完整状态可见的仿真任务。
- P 控制器直接获得目标误差，没有视觉感知或状态估计误差。
- 尚未加入动作噪声、参数随机化、接触、障碍物和真实机器人延迟。
- 尚未训练策略；后续学习算法应至少与随机策略和 P 控制基线比较。
- 下一步需要进一步理解控制频率和动作表示，再建立仿真数据采集与领域随机化。
