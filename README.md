...
AGILEX PIPER 已完成实验 024 的只读反馈和 [实验 025 的参数查询与首次使能反馈验证](experiments/025_piper_safety_query/README.md)。025 不包含故障停止安全性验证；未验证不等于已安全，也不阻塞该实验收尾。接下来先学习关节目标构造与限位预览，实际运动条件另行评估。算法主线仍为 **BC → 闭环失败分析 → PPO → 真实来源数据与序列策略**；机械狗留到第二轮评估。
...
- 实验 025（已完成）：参数查询、单臂固件回复核对及首次六轴使能反馈验证；未发送运动目标，不覆盖故障停止安全性。
