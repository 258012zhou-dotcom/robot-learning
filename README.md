...
当前已经进入**阶段 5：模仿学习与强化学习**。论文推荐嵌入阶段 5 及后续模块，精读和独立论文复现由用户在另一个 Codex 窗口完成；正常讲解和常规实验仍在本项目进行。默认在仿真中验证结果，PIPER 真机实验不作为主线或成果验收要求。
...
阶段 5 从行为克隆（Behavior Cloning, BC）开始，随后进入分布偏移、PPO 和序列策略。概念讲解、基础练习和常规实验继续在本项目中完成；到合适节点再推荐论文，并提供给另一个窗口使用的 [论文精读提示词](prompts/README.md)。若某项论文值得独立做仿真复现，也为外部窗口提供完整复现提示词。
