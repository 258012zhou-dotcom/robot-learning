...
当前已经进入**阶段 5：模仿学习与强化学习**。论文阅读和仿真实验复现直接嵌入阶段 5 及后续模块，用于理解环境、数据、策略和评价的完整软件流程。默认在仿真中验证结果，PIPER 真机实验不作为主线或成果验收要求。
...
- `prompts/`：交给另一个 Codex 执行的论文阅读与仿真实验提示词
- `projects/`：较完整的阶段项目
- `src/`：可以复用的程序代码
- `configs/`：程序和机器人参数
- `tests/`：自动测试
...
阶段 0–4 已完成第一轮，阶段 5 已启动。这里保留成果概览，详细知识清单和接下来的顺序统一放在 [路线图](roadmap.md)，避免多个文件各维护一份过时进度。
...
阶段 5 从行为克隆（Behavior Cloning, BC）开始，随后进入分布偏移、PPO 和序列策略。当前 Codex 会在对应模块选择少量必要论文与可验证的仿真实验，并提供给新 Codex 执行的 [完整提示词](prompts/README.md)，不在本任务中逐步实施实验。

PIPER 历史检查结论已压缩成一份 [未来部署速查](hardware/piper/README.md)，用于快速了解机械臂并避开已知问题。历史实验脚本、测试、配置和逐次记录不再保留在当前版本；实际部署由用户之后决定，历史编号不复用。
