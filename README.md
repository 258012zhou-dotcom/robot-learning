...
当前学习中心是**仿真实验与仿真中的论文复现**，重点理解环境、数据、策略和评价的完整软件流程。默认在仿真中验证结果，PIPER 真机实验不作为主线或成果验收要求。
...
- `hardware/`：按设备分类的真机资料、工具和历史验证记录
- `notes/paper_reproductions/`：独立论文复现项目完成后的总结
- `projects/`：较完整的阶段项目
- `src/`：可以复用的程序代码
- `configs/`：程序和机器人参数
- `tests/`：自动测试
...
进入阶段 5 前，计划先在独立项目中开展几个仿真实验或论文复现，具体任务、平台、论文和顺序由用户之后确定，暂不锁定 SmolVLA。本仓库负责 [仿真实验与论文复现的成果总结](notes/paper_reproductions/README.md)，不提前创建任务或启动训练，阶段 5 暂未启动。

PIPER 的工具、操作与历史验证已整理至 [未来部署快速入门资料](hardware/piper/README.md)：以后准备部署时，可以快速了解机械臂、复用已有工具并避开已知问题，减少重复调试。原 024、025 已按各自范围完成；026 停在准备与诊断，不记为完成。实际部署由用户之后决定，不作为当前仿真实验的必修环节；历史编号不复用。
